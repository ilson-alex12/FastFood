...
public class ContainerChurn extends ContainerFF {
    ...
    public ItemStack transferStackInSlot(EntityPlayer entityPlayer, int slotIndex)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            {
                if (!this.mergeItemStack(itemStack, CHURN_INPUTS + CHURN_OUTPUTS, inventorySlots.size(), false))
                {
                    return null;
                }
            }
            else if (!this.mergeItemStack(itemStack, 0, CHURN_INPUTS + CHURN_OUTPUTS, false))
            {
                return null;
            }
